...
const PdfFiles = () => {
  ...
  useEffect(() => {
    const getFiles = async () => {
      ...
      const directoryPath =
        '/storage/emulated/0/Android/data/com.quickload/files/Documents';
      RNFS.readDir(directoryPath)
        .then(result => {
          const pdfFiles = result.filter(
  ...
  const formatFileName = fileName => {
    ...
    const [prefix, biltyNo, timestamp] = fileName
      .replace('.pdf', '')
      .split('_');
        ...
        <TouchableOpacity
          ...
          <View style={styles.fileInfo}>
            ...
            <View style={styles.textContainer}>
              ...
              <Text style={styles.fileDate}>{fileInfo.datetime}</Text>
            </View>
          </View>
        </TouchableOpacity>
        <IconButton
